...
int main() {
  ...
  // 2. [ PART 2 - Compute sum of extrapolated values for start of sequences ]
  vector<long long> extrapolatedValues;
  for (auto &history : histories) {
    ...
    long long value =
        accumulate(differences.rbegin(), differences.rend(), 0,
                   [](int partialSum, const std::vector<int> &innerVec) {
                     return innerVec.front() - partialSum;
                   });
  ...
  long long sum =
      accumulate(extrapolatedValues.begin(), extrapolatedValues.end(), 0);
  cout << "[ PART 2 ] Extrapolated Value Sum: " << sum << '\n';
