...
int main() {
  ...
  // 2. [ Compute B - Get max number of each rounds ]
  long long sumPower = 0;
  for (const auto &game : games) {
    array<int, 3> setOfBalls = {{0, 0, 0}};
    for (const auto &round : game.rounds) {
      // get minimum number of RBG
      setOfBalls[0] = max(setOfBalls[0], round[0]);
      setOfBalls[1] = max(setOfBalls[1], round[1]);
      setOfBalls[2] = max(setOfBalls[2], round[2]);
    }
    // compute power and sum of powers
    long long gamePower = setOfBalls[0] * setOfBalls[1] * setOfBalls[2];
    sumPower += gamePower;
  }

  cout << "Sum of Game power: " << sumPower << "\n";
